...
class CSetupWizard extends CForm {
	...
	function stage3() {
		...
		return [
			new CTag('h1', true, _('BKM server details')),
			(new CDiv([
				new CTag('p', true, _('Please enter the host name or host IP address and port number of the BKM server, as well as the name of the installation (optional).')),
				$table
			]))->addClass(ZBX_STYLE_SETUP_RIGHT_BODY)
		];
	}
	...
	function stage4() {
		...
		$table->addRow((new CSpan(_('BKM server port')))->addClass(ZBX_STYLE_GREY), $this->getConfig('ZBX_SERVER_PORT'));
		$table->addRow((new CSpan(_('BKM server name')))->addClass(ZBX_STYLE_GREY), $this->getConfig('ZBX_SERVER_NAME'));
	...
	function stage5() {
		...
		if ($error) {
			...
			$message_box = null;
			$message = [
				(new CTag('h1', true, _('Congratulations! You have successfully installed BKM frontend.')))
